...
export class ExternalCalendarFormComponent implements OnInit {
  ...
  private async openMsPopup(): Promise<void> {
    return new Promise<void>((resolve) => {
      const popup = window.open(`/${Environment.CONTEXT}/microsoft`, "Microsoft OAuth", "width=500,height=700,menubar=no,toolbar=no,location=no,status=no,resizable=no,scrollbars=no");
